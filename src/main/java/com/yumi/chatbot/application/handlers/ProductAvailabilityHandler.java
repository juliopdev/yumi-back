...
public class ProductAvailabilityHandler {
  ...
  public ChatResponse handle(ChatbotAI chat, String msg) {
    String prompt = "Extrae de la siguiente frase el **producto** o **categoría** que el usuario desea. " +
        "Responde solo UNA palabra o breve frase, en minúsculas, sin puntuación. " +
        "Ejemplos: \"smartwatch\", \"juguetes\", \"iphone 15\", \"hogar\".\n\n" +
        "Frase: \"" + msg + "\"";
    String term = gemini.call(prompt).trim().toLowerCase();

    if (term.isEmpty() || term.equals("no especifica")) {
    ...
    List<ProductResponse> byName = productService.findByNameContaining(term);
    if (!byName.isEmpty()) {
      return responseFactory.build(chat,
